...
quad_list *load_file (quad_list *root, char *path)
{
    char **object = file_str(path), **t = NULL;
    ...
    for (int i = 0;object[i] != NULL;ptr->next = root, root = ptr, i++) {
        ...
        ptr = tri_one(0, 0, 0, 0);
        sfVertexArray_getVertex(ptr->array, 0)->texCoords.x = atoi(t[9]);
        sfVertexArray_getVertex(ptr->array, 0)->texCoords.y = atoi(t[10]);
        sfVertexArray_getVertex(ptr->array, 1)->texCoords.x = atoi(t[11]);
        sfVertexArray_getVertex(ptr->array, 1)->texCoords.y = atoi(t[12]);
        sfVertexArray_getVertex(ptr->array, 2)->texCoords.x = atoi(t[13]);
        sfVertexArray_getVertex(ptr->array, 2)->texCoords.y = atoi(t[14]);
        for (int i = 0;i < 3;ptr->p1[i][0] = atof(t[i]), i++);
        for (int i = 0;i < 3;ptr->p2[i][0] = atof(t[i + 3]), i++);
        for (int i = 0;i < 3;ptr->p3[i][0] = atof(t[i + 6]), i++);
        ptr->n_texture = atoi(t[15]);
